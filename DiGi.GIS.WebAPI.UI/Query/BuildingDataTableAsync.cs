    ...
    public static partial class Query
    {
        ...
        public static async Task<Table?> BuildingDataTableAsync(this HttpClient? httpClient, int countyId, List<string> columnUniqueIds, Action<string>? log = null, int maxRowCount = int.MaxValue, int commandTimeout = 600, CancellationToken cancellationToken = default)
        {
            ...
            while (true)
            {
                ...
                Table? page = await PageAsync(lastReference);
                if (page is null)
                {
            ...
            // One keyset page, parsed; an empty table for a part with no partition; null after a failed retry.
            async Task<Table?> PageAsync(string? cursor)
            {
                for (int attempt = 0; attempt < 2; attempt++)
                {
                    ...
                    try
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            return await Create.TableAsync(stream, cancellationToken);
                        }
                    }
